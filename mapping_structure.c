	...
	if(ret == 1){
			if(buck == NULL){
				...
				/*need to update the bucket's associated path*/
				 if(nb_update_bucket_path(buck, path) == ERROR){
...
int unregister_server(mappingStructure *str, int sid, char **pathList, int count){
	...
	for(i = 0; i < count; i++){
		  ...
		  if(monitored == 0){//we can delete the subtrees that are not monitored
			  ...
			  if(__unmark_server_subtree(root, tokens, 0, numTok - 1) == -1) return ERROR;
		  }
	  }
	  if(__dfs_clean_subtree(root) == -1) return ERROR;
	  return 0;
  }
